"""
探索版9: 推荐用Qwen text-embedding增强物品表示
用物品特征(i_cat_01~03, i_bucket_01)生成文本描述, 调用embedding API获取语义向量
将语义向量融入GRU4Rec的物品嵌入
"""
import os, sys, time
import numpy as np
...
def get_item_embeddings(item_df, num_items, item2idx):
    """用物品特征生成文本描述, 获取embedding"""
    print("[Embedding] 生成物品文本描述并获取embedding...")
    
    # 检查是否已有缓存的embedding
    cache_path = os.path.join(OUTPUT_DIR, "item_embeddings.npy")
    if os.path.exists(cache_path):
        emb = np.load(cache_path)
        print(f"[Embedding] 从缓存加载: {emb.shape}")
        return torch.FloatTensor(emb).to("cuda" if torch.cuda.is_available() else "cpu")
    
    # 生成物品文本描述
    item_texts = []
    for i in range(num_items + 2):
        if i == 0 or i == 1:
            item_texts.append("padding")
            continue
        iid = None
        for k, v in item2idx.items():
            if v == i:
                ...
                break
        if iid is None:
            item_texts.append("unknown item")
            continue
        row = item_df[item_df["iid"] == iid]
        if len(row) == 0:
            item_texts.append(f"item {iid}")
            continue
        row = row.iloc[0]
        # 用特征生成描述
        text = f"product category {row['i_cat_01']} subcategory {row['i_cat_02']} type {row['i_cat_03']} bucket {row['i_bucket_01']}"
    ...
    try:
        ...
        if not api_key:
            print("[Embedding] 无API key, 用随机初始化替代")
            emb = np.random.randn(num_items + 2, 64).astype(np.float32) * 0.1
        ...
        for start in range(0, len(item_texts), batch_size):
            batch = item_texts[start:start+batch_size]
            try:
                ...
                if resp.status_code == 200:
                    ...
                    print(f"[Embedding] API错误: {resp.code} - {resp.message}")
                    # 用随机替代
                    for _ in batch:
                        all_embeddings.append(np.random.randn(64).astype(np.float32) * 0.1)
            ...
            if (start // batch_size) % 20 == 0:
                print(f"  Embedding {start}/{len(item_texts)}")
            time.sleep(0.1)  # 避免限流
        
        emb = np.array(all_embeddings[:num_items+2], dtype=np.float32)
        np.save(cache_path, emb)
        print(f"[Embedding] 完成: {emb.shape}")
        return torch.FloatTensor(emb).to("cuda" if torch.cuda.is_available() else "cpu")
        
    except ImportError:
        print("[Embedding] dashscope未安装, 用随机初始化")
        emb = np.random.randn(num_items + 2, 64).astype(np.float32) * 0.1
        np.save(cache_path, emb)
        return torch.FloatTensor(emb).to("cuda" if torch.cuda.is_available() else "cpu")


class GRU4RecWithEmb(nn.Module):
    """GRU4Rec + Qwen embedding + 用户特征 + 物品特征"""
    def __init__(self, num_items, embedding_dim=128, hidden_dim=256,
        ...
        # 物品嵌入 (可学习) + Qwen embedding (固定)
        self.item_embedding = nn.Embedding(num_items + 2, embedding_dim, padding_idx=0)
        self.item_emb_proj = nn.Linear(item_emb_dim, embedding_dim)  # 投影Qwen emb到相同维度
    ...
    def forward(self, seq_tensor, seq_lengths, user_features=None, item_features=None, item_qwen_emb=None):
        ...
        # Qwen embedding增强
        if item_qwen_emb is not None:
            qwen_emb = item_qwen_emb[seq_tensor]  # (B, L, emb_dim)
            qwen_proj = self.item_emb_proj(qwen_emb)  # (B, L, D)
            emb = emb + qwen_proj
...
def run_rec(device="cuda", sim_lambda=0.5):
    print("\n" + "=" * 70)
    print("  Task2: GRU4Rec + Qwen embedding + 物品特征 + SimRec")
    print("=" * 70)
    t0 = time.time()
    ...
    # Qwen embedding
    item_qwen_emb = get_item_embeddings(item_df, num_items, item2idx)
    ...
    def evaluate(model):
        ...
        with torch.no_grad():
            for start in range(0, len(val_seqs), 256):
                ...
                scores = model(sb, lb, ub, item_feat_tensor, item_qwen_emb)
                scores[:, 0] = -1e9
    ...
    for model_idx, seed_base in enumerate([42, 142, 242]):
        print(f"\n[GRU4Rec+Qwen #{model_idx+1}] seed={seed_base}")
        torch.manual_seed(seed_base); np.random.seed(seed_base)
        model = GRU4RecWithEmb(num_items, 128, 256, 0.2, max_len,
                               user_feat_dims, item_feat_dims, item_emb_dim=64).to(device)
