"""
探索版4: 分类GCN隐藏层512 + 多DropEdge率集成 + 推荐保持3模型物品特征
"""
import os, sys, time
import numpy as np
import torch
...
def run_cls(device="cuda", n_ensemble=50, hidden_dim=512, lp_weight=0.3):
    print("\n" + "=" * 70)
    print(f"  Task1: GCN{n_ensemble} hidden={hidden_dim} + 伪标签 + lp={lp_weight}")
    print("=" * 70)
    t0 = time.time()
    ...
    # 多DropEdge率集成: 0.1, 0.2, 0.3 交替
    drop_rates = [0.1, 0.2, 0.3]
    print(f"\n[第1轮] GCN {n_ensemble}模型 hidden={hidden_dim}, 多DropEdge率...")
    all_probs = []
    best_val = 0
    for i in range(n_ensemble):
        seed = 42 + i * 10
        dr = drop_rates[i % 3]  # 交替使用不同DropEdge率
        torch.manual_seed(seed); np.random.seed(seed)
        model = build_classification_model("gcn", feat_dim, hidden_dim, num_classes, 2, 0.5, "sym").to(device)
        ...
        for epoch in range(1, 301):
            ...
            adj_tr = drop_edge(adj_sparse, dr)
        ...
        if (i+1) % 10 == 0: print(f"    {i+1}/{n_ensemble} dr={dr} Val={bv:.4f}")
    ...
    print(f"\n[Task1完成] hidden={hidden_dim} 多DropEdge | 耗时: {elapsed/60:.1f}min")
...
def main():
    print("=" * 70)
    print("  探索版4: GCN hidden=512 + 多DropEdge率集成")
    print("=" * 70)
    t0 = time.time()
    device = "cuda" if torch.cuda.is_available() else "cpu"
    cls_val = run_cls(device=device, n_ensemble=50, hidden_dim=512, lp_weight=0.3)
    elapsed = time.time() - t0
