"""
探索版6: 分类sym+rw双归一化集成 + 推荐embedding_dim=128
分类: 25模型sym + 25模型rw 按验证分数加权融合
推荐: embedding_dim=128 (更大容量) + 物品特征 + 3模型集成
"""
import os, sys, time
import numpy as np
import pandas as pd
...
def run_cls(device="cuda", n_per_norm=25, lp_weight=0.3):
    print("\n" + "=" * 70)
    print(f"  Task1: sym+rw双归一化集成 ({n_per_norm}x2={n_per_norm*2}模型)")
    print("=" * 70)
    t0 = time.time()
    ...
    # 两种归一化
    adj_sym = preprocess_adj(adj, add_self_loops=True, normalization="sym")
    adj_rw = preprocess_adj(adj, add_self_loops=True, normalization="rw")
    adj_sym_sp = sparse_csr_to_torch_sparse(adj_sym, device=device)
    adj_rw_sp = sparse_csr_to_torch_sparse(adj_rw, device=device)
    ...
    def train_ensemble(norm_type, adj_sp, n_models, seed_base):
        all_probs = []
        best_val = 0
        for i in range(n_models):
            ...
            for epoch in range(1, 301):
                ...
                if epoch % 10 == 0:
                    model.eval()
                    with torch.no_grad():
                        vp = model(features_t, adj_sp)[val_t].argmax(1)
                        va = (vp == labels_t[val_t]).float().mean().item()
                        if va > bv:
                            bv = va
                            best_probs = F.softmax(model(features_t, adj_sp)[test_t], dim=1).cpu().numpy()
            all_probs.append(best_probs)
            if va > best_val: best_val = va
            if (i+1) % 10 == 0: print(f"    {norm_type} {i+1}/{n_models} Val={bv:.4f}")
        return np.mean(all_probs, axis=0), best_val

    # sym集成
    print(f"\n[sym集成] {n_per_norm}模型...")
    sym_probs, sym_val = train_ensemble("sym", adj_sym_sp, n_per_norm, 42)
    print(f"  sym Val={sym_val:.4f}")

    # rw集成
    print(f"\n[rw集成] {n_per_norm}模型...")
    rw_probs, rw_val = train_ensemble("rw", adj_rw_sp, n_per_norm, 42 + n_per_norm * 10)
    print(f"  rw Val={rw_val:.4f}")

    # 按验证分数加权融合
    total_val = sym_val + rw_val
    sym_w = sym_val / total_val
    rw_w = rw_val / total_val
    print(f"\n[双归一化融合] sym_w={sym_w:.3f}, rw_w={rw_w:.3f}")
    mixed = sym_w * sym_probs + rw_w * rw_probs
    ...
    print(f"\n[第2轮] 伪标签+sym+rw混合重训...")
    pseudo_probs = []
    for i in range(n_per_norm):
        for norm_type, adj_sp, seed_off in [("sym", adj_sym_sp, 0), ("rw", adj_rw_sp, n_per_norm * 10)]:
            seed = 500 + i * 10 + seed_off
            torch.manual_seed(seed); np.random.seed(seed)
            model = build_classification_model("gcn", feat_dim, 256, num_classes, 2, 0.5, norm_type).to(device)
            opt = torch.optim.Adam(model.parameters(), lr=0.01, weight_decay=5e-4)
            sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=300, eta_min=1e-5)
...
def run_rec(device="cuda", sim_lambda=0.5, embedding_dim=128, hidden_dim=256):
    print("\n" + "=" * 70)
    print(f"  Task2: GRU4Rec emb={embedding_dim} hidden={hidden_dim} + 物品特征 + 3模型集成")
    print("=" * 70)
    t0 = time.time()
    ...
    for model_idx, seed_base in enumerate([42, 142, 242]):
        print(f"\n[GRU4Rec #{model_idx+1}] emb={embedding_dim} hidden={hidden_dim} seed={seed_base}")
        ...
        print(f"  GRU4Rec #{model_idx+1} Val={best_val:.4f}")
...
def main():
    ...
    print("=" * 70)
    print("  探索版6: sym+rw双归一化分类 + emb=128推荐")
    print("=" * 70)
    t0 = time.time()
    ...
    if args.task in ("both", "cls"):
        cls_val = run_cls(device=device, n_per_norm=25, lp_weight=0.3)
    ...
    print(f"\n{'='*70}")
    print(f"  分类Val={cls_val:.4f} | 推荐Val={rec_val:.4f} | 预估test总分={est_total:.4f}")
