...
- IF 首轮baseline完成且准确率>0.65 THEN 启动伪标签(置信度>0.8)
- IF 伪标签有效(准确率提升) THEN 继续伪标签, 降低阈值(0.8→0.7→0.6→0.5)
- IF 伪标签后准确率下降 THEN 回退到上一轮配置
- IF 已做3轮伪标签 THEN 转向其他优化方向(标签传播权重/集成数量)
