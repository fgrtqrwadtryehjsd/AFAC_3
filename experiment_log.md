...
| 提交 | 时间 | 总分 | 分类 | 推荐 | 关键改进 |
|------|------|------|------|------|----------|
| #1 | 06/25 11:27 | 0.5163 | 0.5631 | 0.4695 | 初始 Agent (GraphSAGE + GRU4Rec, LLM决策3轮) |
| #2 | 06/25 14:29 | 0.4884 | 0.7121 | 0.2646 | GCN+DropEdge+8集成, 推荐加流行度(失败) |
| #3 | 06/25 14:48 | 0.5913 | 0.7121 | 0.4705 | 修复推荐(关闭流行度, 回归纯模型) |
| #4 | 06/25 23:22 | 待测 | ~0.70 | ~0.59 | 15集成+序列增强+Agent轨迹日志 |
...
### 推荐任务
1. **CE损失优于BPR** (0.59 vs 0.48)
2. **流行度混合有害** (尺度不匹配, 0.47→0.26)
3. **序列增强有帮助** (模拟短序列分布, 36K→89K)
4. **GRU4Rec优于SASRec** (稀疏新用户场景)
5. **验证-测试差距大** (0.59→0.47), 核心问题是训练序列长(46)而测试短(6.25)
6. **增大模型容量未必有效** (边际收益递减)
